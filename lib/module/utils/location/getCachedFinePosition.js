...
const getCachedFinePosition = () => {
  return new Promise((resolve, reject) => {
    ...
    const getPosition = () => {
      Geolocation.getCurrentPosition(position => onSuccess(resolve, position), error => onGeolocationError(reject, error), OptionsForCachedFine);
    };
      ...
      if (checkResult === RESULTS.GRANTED) {
        ...
        request(permissions.bestLocation).then(requestResult => {
          if (requestResult === RESULTS.GRANTED) {
            ...
            Alert.alert('Permission Warning', 'This app needs location permission to ALWAYS to work', [{
              text: 'Open settings',
