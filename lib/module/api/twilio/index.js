...
import { getTwilioAccessTokenFromStorage, isTokenExpired, refreshTwilioAccessToken, saveTwilioAccessToken } from '../../utils/token';
...
let client;
const getToken = () => axios.post(`${endpoints.twilio}/auth`).then(response => response.data);
...
export default {
  ...
  getMessages: async incidentId => {
    ...
    return map(messagePaginator.items, message => convertToGiftedChatMessage(message, members)).reverse();
  },
