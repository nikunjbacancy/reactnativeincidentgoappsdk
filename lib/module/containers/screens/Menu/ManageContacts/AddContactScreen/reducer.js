...
import { FILTER_CONTACTS, GET_CONTACTS_FAILURE, GET_CONTACTS_SUCCESS } from './constants';
export const initialState = {
  contacts: [],
  filteredContacts: [],
  permissionStatus: RESULTS.UNAVAILABLE
};
const addContactsScreenReducer = produce((draft, action) => {
  switch (action.type) {
    ...
    case FILTER_CONTACTS:
      draft.filteredContacts = filter(draft.contacts, item => startsWith(item.name, action.payload));
      break;
    default:
  }
}, initialState);
