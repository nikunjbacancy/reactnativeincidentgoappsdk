...
const myTipsScreenReducer = produce((draft, action) => {
  switch (action.type) {
    ...
    case GET_TIPS_SUCCESS:
      if (action.payload.paging) {
        draft.tips[action.payload.tipStatus].data = uniqBy([...draft.tips[action.payload.tipStatus].data, ...action.payload.data], 'id');
      } else {
