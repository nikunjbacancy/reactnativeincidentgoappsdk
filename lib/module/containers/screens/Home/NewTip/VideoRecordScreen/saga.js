...
import { addIncidentVideo, createIncidentFailure, createIncidentSuccess, createTipWithChatFailure, createTipWithChatSuccess, deleteIncidentFailure, deleteIncidentSuccess, getLastTipOrganizationFailure, getLastTipOrganizationSuccess, getUserOrganizationsFailure, getUserOrganizationsSuccess, hideCancelIncidentModal, uploadVideoFailure, uploadVideoSuccess } from './actions';
import { CREATE_INCIDENT_REQUEST, CREATE_TIP_WITH_CHAT_REQUEST, DELETE_INCIDENT_REQUEST, GET_LAST_ACTIVE_TIP_ORGANIZATION_REQUEST, GET_USER_ORGANIZATIONS_REQUEST, UPLOAD_VIDEO_REQUEST } from './constants';
export function* createIncident() {
  try {
    const response = yield call(api.incidents.createIncident);
...
function* getAmazonS3Options(filePath) {
  let incident = yield select(makeSelectNewTipIncident());
  try {
    if (!incident) {
    ...
    const amazonOptionsResponse = yield call(api.incidents.getAmazonOptions, incident, filePath);
...
function* deleteIncident() {
  try {
    const incident = yield select(makeSelectNewTipIncident());
    if (incident) {
...
function* getLastActiveTipOrganization({
  ...
  try {
    const organizationResponse = yield call(api.organizations.getOrganization, payload);
...
export default function* newTipScreenSaga() {
  ...
  yield takeLatest(GET_LAST_ACTIVE_TIP_ORGANIZATION_REQUEST, getLastActiveTipOrganization);
}
