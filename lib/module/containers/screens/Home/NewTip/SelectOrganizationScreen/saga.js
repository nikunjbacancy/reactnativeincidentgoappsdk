...
import { makeSelectNewTipIncidentExpired, makeSelectNewTipIncidentLocation } from '../../../../../containers/app/selectors';
import Screens from '../../../../../containers/providers/RoutesProvider/screens';
import { call, put, select, takeLatest } from 'redux-saga/effects';
import { handleError } from '../../../../../utils/error';
import NavigatorService from '../../../../../utils/navigation';
import { addSelectedOrganizationFailure, addSelectedOrganizationSuccess, getIntersectOrganizationsFailure, getIntersectOrganizationsSuccess } from './actions';
import { GET_INTERSECT_ORGANIZATIONS_REQUEST, SELECT_INCIDENT_ORGANIZATION_REQUEST } from './constants';
import { makeSelectSelectedOrganization } from './selectors';
function* getIntersectOrganizations() {
  try {
    const location = yield select(makeSelectNewTipIncidentLocation());
    ...
    if (response.data.organizations.length === 1) {
      ...
      yield call(NavigatorService.navigate, Screens.Home.NewTip.SelectOrganization);
    }
  } catch (error) {
...
function* addSelectedOrganization() {
  ...
  const incidentExpired = yield select(makeSelectNewTipIncidentExpired());
  if (!incidentExpired) {
...
export default function* organizationNotifyScreenSaga() {
  ...
  yield takeLatest(SELECT_INCIDENT_ORGANIZATION_REQUEST, addSelectedOrganization);
}
