...
const timedEscortScreenReducer = produce((draft, action) => {
  switch (action.type) {
    ...
    case GET_INTERSECT_ORGANIZATIONS_SUCCESS:
      ...
      draft.organizationGroups = map(action.payload.organizationGroups, group => {
    ...
    case TOGGLE_SELECT_ORGANIZATION:
      ...
      draft.organizationGroups = map(draft.organizationGroups, organization => {
