...
import { Config, Id } from 'incident-code-core';
...
} from '../screens/Home/NewTip/VideoRecordScreen/constants';
...
} from '../screens/Menu/ManageContacts/ContactScreen/constants';
...
const appReducer = produce((draft: Draft<AppState>, action: AppActionTypes) => {
  switch (action.type) {
    case FINISH_WELCOME:
    ...
    case UPDATE_USER_EMAIL:
      assign(draft.user, { email: action.payload });
      break;
    case UPDATE_USER_PORTRAIT:
      assign(draft.user, { portraitUrl: action.payload });
      break;
    ...
    case DELETE_USER_ORGANIZATION_SUCCESS:
      draft.user.organizations = map(action.payload, 'id') as Id[];
      break;
    case TOGGLE_CONTACT_NOTIFICATION_SUCCESS:
      draft.user.contacts?.splice(
        findIndex(draft.user.contacts, { id: action.payload.id }),
        1,
        action.payload,
      );
      break;
    case DELETE_CONTACT_SUCCESS:
      draft.user.contacts?.splice(
        findIndex(draft.user.contacts, { id: action.payload }),
        1,
      );
      break;
    case ADD_CONTACT_SUCCESS:
      draft.user.contacts?.push(action.payload);
      break;
