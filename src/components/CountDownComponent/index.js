...
const myModuleEventEmitter = new NativeEventEmitter(CounterModule);
...
class CountDown extends React.Component {
  ...
  componentDidMount() {
    ...
    this.countdownSuccess = myModuleEventEmitter.addListener(
      'countdownEvent',
      (result) => {
        // console.log('countdown time in seconds 1 :', result);
        let timer = result / 1000
        this.updateTimerForAndroid(timer)
      }
    );
  ...
  componentWillReceiveProps(nextProps) {
    if (Platform.OS == 'android') {
      ...
      if (nextProps.running) {
        if (this.state.isBackgroundServiceRunning === false) {
          this.startBackgroundService()
          this.setState({ isBackgroundServiceRunning: true })
        }
      }
    }
  ...
  startBackgroundService = async () => {
    ...
    NativeModules.CounterModule.startCounter(this.props.until)
  }

  stopService = () => {
    NativeModules.CounterModule.stopCounter(-1);
  }
  ...
  _handleAppStateChange = currentAppState => {
    ...
    if (currentAppState === 'background') {
      if (Platform.OS == 'ios') {
        if (this.state.isAppInBackground == false) {
          ...
          BackgroundTimer.runBackgroundTimer(() => {
            ...
            1000);
          this.setState({ isAppInBackground: true });
        }
      }
      else {
  ...
  updateTimer = () => {
    ...
    console.log("-this.state.until->", this.state.until)
